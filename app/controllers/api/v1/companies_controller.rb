class Api::V1::CompaniesController < ApplicationController
  def index
    companies = Company.left_outer_joins(:deals)
      .group(:id)
      .select(Arel.sql("companies.*, COALESCE(SUM(deals.amount), 0) as total_amount"))
    ...
    companies.having!("SUM(deals.amount) > ?", params['minimumDealAmount'].to_i) if params['minimumDealAmount'] && params['minimumDealAmount'].to_i
